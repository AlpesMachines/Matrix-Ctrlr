...
/////////////////////////////////////////////////////////////////////////////
//  Send a sysex depending on interface chosen
/////////////////////////////////////////////////////////////////////////////
void MIDI_SendSysex(unsigned char interface, byte* sysex)
{
...
/////////////////////////////////////////////////////////////////////////////
// Send a matrix bank change sysex request
/////////////////////////////////////////////////////////////////////////////
void MIDI_SendPatchBank(unsigned char interface, unsigned char bank)
{
  byte sysex[6] = { 0xf0, 0x10, 0x06, 0x0A, bank, 0xF7};
...
void MIDI_SendPatchProgram(unsigned char interface, unsigned char program)
{
  switch (interface) {
    ...
    case INTERFACE_SERIAL3:
      MIDI3.sendProgramChange(program, MIDI_CHANNEL + device - 0x0a);
      break;
...
#if DEBUG_midi
  Serial.print(F("MIDI_SendPatchProgram_ ")); Serial.print (F("program = ")); Serial.println(program, DEC);
  Serial.println();
#endif
...
void MIDI_SendPatchNumber(unsigned char interface, unsigned char bank, unsigned char program)
{
  ...
  switch (interface) {
    ...
    case INTERFACE_SERIAL3:
      MIDI3.sendControlChange (0x00, bank, MIDI_CHANNEL + device - 0x0a);
      MIDI3.sendControlChange (0x20, 0, MIDI_CHANNEL + device - 0x0a);
      MIDI3.sendProgramChange(program, MIDI_CHANNEL + device - 0x0a);
      break;
...
#if DEBUG_midi
  Serial.print(F("MIDI_RequestSinglePatch ")); Serial.print (F("ProgramNumberReq = ")); Serial.print(ProgramNumberReq, DEC); Serial.print (F(" number = ")); Serial.println(number, DEC);
  Serial.println();
#endif
}
...
#if DEBUG_midi
  Serial.print(F("MIDI_RequestGP ")); Serial.print (F("interface = ")); Serial.println(interface, HEX);
  Serial.println();
#endif
}
...
#if DEBUG_midi
  Serial.print(F("MIDI_RequestEditBuffer ")); Serial.print (F("interface = ")); Serial.print(interface, HEX); Serial.print (F(" number = ")); Serial.println(number, DEC);
  Serial.println();
#endif
...
#if DEBUG_midi
  Serial.print(F("MIDI_BankRequest_ ")); Serial.print (F("interface = ")); Serial.print(interface, HEX); Serial.print (F(" bank_req = ")); Serial.println(bank_req, DEC);
  Serial.println();
#endif
}
...
void MIDI_SendVoiceParam(unsigned char interface, unsigned char param, unsigned char value, bool midiThru)
{
  ...
  if (param != lastparam || value != lastvalue)
  {
...
#if DEBUG_serialout
    // print debug sysex +
    Serial.print(F("ARD SERIAL_PORT = ")); Serial.print(interface, DEC); Serial.print(F(" MIDI_SendVoiceParam - SysEx[]= "));
    ...
    if (param == SX_UNISON_DETUNE)
    {
      ...
      {
        case INTERFACE_SERIAL1:
          MIDI1.sendControlChange( 94, value, MIDI_CHANNEL + interface - 1);
          break;
        case INTERFACE_SERIAL2:
          MIDI2.sendControlChange( 94, value, MIDI_CHANNEL + interface - 2);
          break;

          //        case INTERFACE_SERIAL3:
          //          MIDI3.sendControlChange( 94, value, MIDI_CHANNEL + interface - 3);
          //          break;

#if SOFTSERIAL_ENABLED
        case INTERFACE_SERIAL4:
          MIDI4.sendControlChange( 94, value, MIDI_CHANNEL + interface - 4);
          break;

        case INTERFACE_SERIAL5:
          MIDI5.sendControlChange( 94, value, MIDI_CHANNEL + interface - 5);
          break;
#endif
        default:
          break;
      }
      // send corresponding CC to DAW
      MIDI3.sendControlChange( 94, value, MIDI_CHANNEL + interface - 1);
    }
    else
    {
      switch (interface)
...
void MIDI_HandleMatrixModTransmitDelay(unsigned char interface)
{
  ...
  if (encoder_send_counter > 300)   // delay about a 3rd second : 5120
  {
    if (matrix_modele == matrix_6)
    {
      ...
      SendEditBuffer(interface); // BUG of M6 : can't handle those sysex modmatrix messages :( #hack
    }
    else
    {
      switch (interface) {
...
#if SOFTSERIAL_ENABLED
    case INTERFACE_SERIAL4:
      MIDI4.sendControlChange(0x02, value, MIDI_CHANNEL + 3);
      break;

    case INTERFACE_SERIAL5:
      MIDI5.sendControlChange(0x02, value, MIDI_CHANNEL + 4);
      break;
#endif
...
void MIDI_Send_UNISONDETUNE(unsigned char interface, unsigned char value)
{
  switch (interface) {
    ...
    case INTERFACE_SERIAL3:
      MIDI3.sendControlChange(94, value, MIDI_CHANNEL + 2);
      break;

#if SOFTSERIAL_ENABLED
    case INTERFACE_SERIAL4:
      MIDI4.sendControlChange(94, value, MIDI_CHANNEL + 3);
      break;

    case INTERFACE_SERIAL5:
      MIDI5.sendControlChange(94, value, MIDI_CHANNEL + 4);
      break;
#endif
