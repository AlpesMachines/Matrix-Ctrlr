...
void UI_Display_Filter()
{
  if (SoftPanel.Page == SOFT_PAGE1)
  {
    ...
    if ((1 << 0) & EditBuffer[EB_FILTER_LEVERMOD]) // is pitch bend bit 0 set?
    {
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + LCD_Offset);
      LCD_PrintCString(F("ON "));
    }

    if ((1 << 1) & EditBuffer[EB_FILTER_LEVERMOD]) // is mod wheel bit 1 set?
    {
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
      LCD_PrintCString(F("ON "));
    }

    if ((1 << 0) & EditBuffer[EB_FILTER_KEYBOARDMOD]) // is portamento bit 0 set?
    {
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);
      LCD_PrintCString(F("ON "));
    }

    if ((1 << 1) & EditBuffer[EB_FILTER_KEYBOARDMOD]) // is kbd tracking bit 1 set?
    {
  ...
  else if (SoftPanel.Page == SOFT_PAGE2)// page 2
  {
    ...
    // vca1 amount
    LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
    LCD_PrintBCD2(EditBuffer[EB_VCA1_AMOUNT]);

    // vca1 velocity
    LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);
    LCD_Display7BitSigned(EditBuffer[EB_VCA1_AMPBYVELOCITY]);

    // vca2-env2
    LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 13 + LCD_Offset);
    LCD_Display7BitSigned(EditBuffer[EB_VCA2_AMPBYENV2]);
...
void UI_Handle_Filter()
{
  ...
  if (SoftPanel.Page == SOFT_PAGE1)
  {
    switch (SoftPanel.Button)
    {
      ...
      case SOFT_EDIT_1:
        EditBuffer[EB_FILTER_LEVERMOD]  ^= (1 << 0); // toggle pitch bend bit 0
        param = SX_FILTER_LEVERMOD;
        value = EditBuffer[EB_FILTER_LEVERMOD];
        MIDI_SendVoiceParam(INTERFACE_SERIAL, param, value, true);
        break;

      case SOFT_EDIT_2:
        EditBuffer[EB_FILTER_LEVERMOD] ^= (1 << 1); // toggle mod wheel bit 1
        param = SX_FILTER_LEVERMOD;
        value = EditBuffer[EB_FILTER_LEVERMOD];
        MIDI_SendVoiceParam(INTERFACE_SERIAL, param, value, true);
        break;
      ...
      case SOFT_EDIT_3:
        EditBuffer[EB_FILTER_KEYBOARDMOD]  ^= (1 << 0); // toggle portamento bit 0
        param = SX_FILTER_KEYBOARDMOD;
        value = EditBuffer[EB_FILTER_KEYBOARDMOD];
        MIDI_SendVoiceParam(INTERFACE_SERIAL, param, value, true);
        break;

      case SOFT_EDIT_4:
        EditBuffer[EB_FILTER_KEYBOARDMOD]  ^= (1 << 1); // toggle keyboard tracking bit 1
        param = SX_FILTER_KEYBOARDMOD;
        value = EditBuffer[EB_FILTER_KEYBOARDMOD];
        MIDI_SendVoiceParam(INTERFACE_SERIAL, param, value, true);
        break;
  ...
  else if ( SoftPanel.Page == SOFT_PAGE2) //////////////////////////////////////////// page 2 ///////////////////////////
  {
    ...
    if (SoftPanel.EncoderValue != 0)
    {
      switch (SoftPanel.EditButton) // n'envoie pas de sysex ! à corriger pour OS 2.14 et 1.20
      {
        case Edit2:
          // vca amount
          SetEncoderValue(&EditBuffer[EB_VCA1_AMOUNT], 63);
          param = SX_VCA1_AMOUNT;
          value = EditBuffer[EB_VCA1_AMOUNT];
          MIDI_SendDelayedVoiceParam(param, value);
          break;

        case Edit3:
          // vca velocity
          SetEncoder7Signed(&EditBuffer[EB_VCA1_AMPBYVELOCITY]);
          param = SX_VCA1_AMPBYVELOCITY;
          value = EditBuffer[EB_VCA1_AMPBYVELOCITY];
          MIDI_SendDelayedVoiceParam(param, value);
          break;

        case Edit4:
          // vca envelope
          SetEncoder7Signed(&EditBuffer[EB_VCA2_AMPBYENV2]);
          param = SX_VCA2_AMPBYENV2;
          value = EditBuffer[EB_VCA2_AMPBYENV2];
    ...
    switch (SoftPanel.Button)
    {
      ...
      case SOFT_EDIT_2: // toggle filter cutoff tweak mode, sysex, or continuous controller, lever 3 (breath)
        ...
        if (FilterCutoffMode == FILTER_FREQ_MODE_CCONTROL)
        {
          ...
          {
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Amount ]       = 63; // max
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Source ]       = MMOD_SRC_LEVER3;
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Destination ]  = MMOD_DEST_FILTER_FREQ;
            // set filter to zero for maximum effect
            EditBuffer[ EB_FILTER_FREQ] = 0;
            MIDI_SendVoiceParam(INTERFACE_SERIAL, SX_FILTER_FREQ, 0, true);
            MIDI_SendMatrixModParam(filter_cc_modbus, EditBuffer[EB_MmodMap[filter_cc_modbus].Source], EditBuffer[EB_MmodMap[filter_cc_modbus].Destination], 63); // max
          }
          else
          {
        ...
        else // free up the bus
        {
          if (filter_cc_modbus < 10)
          {
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Amount ]       = 0; // off
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Source ]       = MMOD_SRC_UNUSED;
            EditBuffer[ EB_MmodMap[filter_cc_modbus].Destination ]  = MMOD_DEST_UNUSED;
            MIDI_SendMatrixModParam(filter_cc_modbus, EditBuffer[EB_MmodMap[filter_cc_modbus].Source], EditBuffer[EB_MmodMap[filter_cc_modbus].Destination], EditBuffer[ EB_MmodMap[filter_cc_modbus].Amount ]  );
          }
...
unsigned char getFreeModBus(void)
{
  ...
  for (i = 0; i < 10; i++)
  {
    if ( EditBuffer[ EB_MmodMap[i].Destination ] == 0)
    {
      bus = i;
      break;
    }
