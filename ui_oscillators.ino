  ...
  switch (SoftPanel.Page)
  {
    case SOFT_PAGE1: // first page
      if (SoftPanel.IsNewPage)
      {
        ...
        LCD_PrintCString(F("DCO BND VIB PORT KBD"));
      }

      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + LCD_Offset);

      // set up a default display
      LCD_PrintCString(F("1   OFF OFF OFF  OFF"));

      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + LCD_Offset);
      LCD_PrintBCD2(SoftPanel.Selected_Osc + 1);

      if ((1 << 0) & EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod]) // is pitch bend bit 0 set?
      {
        LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
        LCD_PrintCString(F("ON "));
      }

      if ((1 << 1) & EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod]) // is vibrato bit 1 set?
      {
        LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);
        LCD_PrintCString(F("ON "));
      }

      if ((1 << 0) & EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod]) // is portamento bit 0 set?
      {
        LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 12 + LCD_Offset);
        LCD_PrintCString(F("ON "));
      }
      
      if ((1 << 1) & EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod]) // is portamento bit 1 set?
      {
    ...
    case SOFT_PAGE2:
      ...
      // lfo amp modulation
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
      LCD_Display7BitSigned(EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].AmpModulation]);

      // lfo speed modulation
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 13 + LCD_Offset);
      LCD_Display7BitSigned(EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SpeedModulation]);
    ...
    case SOFT_PAGE3:
      ...
      // lfo trigger mode
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
      //      LCD_PrintCString(LFO_Trigger_Mode_Names[EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].TriggerMode]]);
      lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(LFO_Trigger_Mode_Names[EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].TriggerMode]]))));

      // lfo sample source
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);
      //      LCD_PrintCString(MMOD_Sources[EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SampledSource]]);
      lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(MMOD_Sources[EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SampledSource]]))));

      // lfo lag enable
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 13);
      if (EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].LagEnable] == 0)
      ...
      LCD_PrintBCD2(EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].RetriggerPoint]);
    ...
    case SOFT_PAGE1:
      ...
      switch (SoftPanel.Button)            {
        ...
        case SOFT_EDIT_2: // pitch bend, toggle on/off, levermod
          EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod] ^= (1 << 0); // toggle bit 0
          param = SoftPanel.Selected_Osc == 0 ? SX_OSC1_LEVERMOD : SX_OSC2_LEVERMOD;
          value = EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod];
          break;

        case SOFT_EDIT_3: // mod wheel, toggle on/off, levermod
          EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod] ^= (1 << 1); // toggle bit 1
          param = SoftPanel.Selected_Osc == 0 ? SX_OSC1_LEVERMOD : SX_OSC2_LEVERMOD;
          value = EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].LeverMod];
          break;

        case SOFT_EDIT_4: // portamento, toggle on/off, keyboardmod
          EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod] ^= (1 << 0); // toggle bit 0
          param = SoftPanel.Selected_Osc == 0 ? SX_OSC1_KEYBOARDMOD : SX_OSC2_KEYBOARDMOD;
          value = EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod];
          break;

        case SOFT_EDIT_5: // keybd tracking, toggle on/off, keyboardmod
          EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod] ^= (1 << 1); // toggle bit 1
          param = SoftPanel.Selected_Osc == 0 ? SX_OSC1_KEYBOARDMOD : SX_OSC2_KEYBOARDMOD;
          value = EditBuffer[EB_OscillatorMap[SoftPanel.Selected_Osc].KeyboardMod];
          break;
    ...
    case SOFT_PAGE2:
      ...
      if (SoftPanel.EncoderValue != 0)
      {
        switch (SoftPanel.EditButton)
        {
          case Edit2:
            // lfo amp modulation
            SetEncoder7Signed(&EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].AmpModulation]);
            param = (SoftPanel.Selected_LFO == LFO1) ? SX_LFO1_AMPMODULATION : SX_LFO2_AMPMODULATION;

            MIDI_SendDelayedVoiceParam(param, EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].AmpModulation]);

            break;

          case Edit4:
            // lfo speed modulation
            SetEncoder7Signed(&EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SpeedModulation]);
            param = (SoftPanel.Selected_LFO == LFO1) ? SX_LFO1_SPEEDMODULATION : SX_LFO2_SPEEDMODULATION;
            MIDI_SendDelayedVoiceParam(param, EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SpeedModulation]);
            break;
    ...
    case SOFT_PAGE3:
      ...
      switch (SoftPanel.Button)
      {
        ...
        case SOFT_EDIT_2: // lfo trigger mode
          ...
          // increment the value
          value = (SoftPanel.Selected_LFO == LFO1) ? EditBuffer[EB_LFO1_TRIGGERMODE] : EditBuffer[EB_LFO2_TRIGGERMODE];
          value++;
          if (value > 3)
            value = 0;
          ...
          if (SoftPanel.Selected_LFO == LFO1)
            EditBuffer[EB_LFO1_TRIGGERMODE] = value;
          else
            EditBuffer[EB_LFO2_TRIGGERMODE] = value;
        ...
        case SOFT_EDIT_4: // lfo lag enable
          ...
          if (SoftPanel.Selected_LFO == LFO1)
          {
            ...
            EditBuffer[EB_LFO1_LAGENABLE] = (EditBuffer[EB_LFO1_LAGENABLE] == ON) ? OFF : ON;
            value = EditBuffer[EB_LFO1_LAGENABLE];
          }
          else
          {
            param = SX_LFO2_LAGENABLE;
            EditBuffer[EB_LFO2_LAGENABLE] = (EditBuffer[EB_LFO2_LAGENABLE] == ON) ? OFF : ON;
            value = EditBuffer[EB_LFO2_LAGENABLE];
          }
      ...
      if (SoftPanel.EncoderValue != 0)
      {
        switch (SoftPanel.EditButton)
        {
          case Edit3: // lfo sample source
            ...
            SetEncoderValue(&EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SampledSource], 20);
            param = (SoftPanel.Selected_LFO == LFO1) ? SX_LFO1_SAMPLEDSOURCE : SX_LFO2_SAMPLEDSOURCE;

            MIDI_SendVoiceParam(INTERFACE_SERIAL, param, EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].SampledSource], true);

            break;

          case Edit5: // lfo retrigger point

            SetEncoderValue(&EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].RetriggerPoint], 63);
            param = (SoftPanel.Selected_LFO == LFO1) ? SX_LFO1_RETRIGGERPOINT : SX_LFO2_RETRIGGERPOINT;

            MIDI_SendVoiceParam(INTERFACE_SERIAL, param, EditBuffer[EB_LfoMap[SoftPanel.Selected_LFO].RetriggerPoint], true);
