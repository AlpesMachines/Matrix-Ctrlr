...
/*
  ...
  else
  {
    ...
    // EditBuffer[device][pgm_read_word_near(&singlePatchDataFormatEB[controlNumber][3])] = value;
    //  }
    //}
    //    MIDI_SendVoiceParam(channel, controlNumber, value, false);
    //    EditBuffer[device][controlNumber] = value; // update the edit buffer ! ^^
  #if DEBUG_router
    Serial.print(F("ControlChange MIDI_SendVoiceParam/EditBuffer[device] : ")); Serial.print(channel, DEC); Serial.print(F(" ")); Serial.print (controlNumber, DEC); Serial.print(F(" ")); Serial.println(value, DEC);
    Serial.print(F("EditBuffer[device][")); Serial.print(controlNumber, HEX); Serial.print(F("] = ")); Serial.println(value, HEX);
  #endif
...
void HandleProgramChange(byte channel, byte program)
{
  ...
  if ((channel >= MIDI_CHANNEL) && (channel <= MIDI_CHANNEL + 4) && (program < 100))
  {
    ...
    /*    // then send midi sysex !
        if (channel == MIDI_CHANNEL)
        {
          SendEditBuffer(channel + Matrix_Device_A - MIDI_CHANNEL, INTERFACE_SERIAL1);
        }
        if (channel == MIDI_CHANNEL + 1)
          SendEditBuffer(channel + Matrix_Device_A - MIDI_CHANNEL, INTERFACE_SERIAL2);
        if (channel == MIDI_CHANNEL + 2)
          SendEditBuffer(channel + Matrix_Device_A - MIDI_CHANNEL, INTERFACE_SERIAL4);
        if (channel == MIDI_CHANNEL + 3)
          SendEditBuffer(channel + Matrix_Device_A - MIDI_CHANNEL, INTERFACE_SERIAL5);
