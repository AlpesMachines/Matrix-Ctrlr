  ...
  switch (SoftPanel.Page) {
    ...
    case SOFT_PAGE2: // name the patch
      ...
      */
      ...
      lcd.print(F(" ")); // bug of blinking cursor solved
      break;
...
void UI_Handle_Edit(void)
{
  ...
  else if (SoftPanel.Page == SOFT_PAGE2) //////////////////////////////////////////////////////////////////  PAGE 2 /////////
  {
    ...
    if (EditBuffer[device][curseur] >= 90) // limit letters to human readable range (cf ASCII table)
      EditBuffer[device][curseur] = 90;
    if (EditBuffer[device][curseur] < 32)
      EditBuffer[device][curseur] = 32;

    EditBuffer[device][curseur] += SoftPanel.EncoderValue;
    if (curseur > 7)
      curseur = 0;

    lcd.setCursor(8 + curseur, 1);
    lcd.blink();
