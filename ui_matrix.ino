...
void UI_Display_Matrix()
{
  switch (SoftPanel.Page)
  {
    case SOFT_PAGE1:
      ...
      //LCD_PrintCString(MMOD_Sources[EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Source]]);
      lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(MMOD_Sources[EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Source]]))));
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);
      //LCD_PrintCString(MMOD_Destinations[EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Destination]]);
      lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(MMOD_Destinations[EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Destination]]))));
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 17 + LCD_Offset);
      LCD_Display7BitSigned(EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Amount]);
      //            LCD_PrintBCD3(EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Amount]);
      break;
    ...
    case SOFT_PAGE2:
      ...
      if (SoftPanel.Selected_Ramp == RAMP1)
        LCD_PrintBCD2(EditBuffer[EB_RAMP1_RATE]);
      else
        LCD_PrintBCD2(EditBuffer[EB_RAMP2_RATE]);
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 8 + LCD_Offset);

      if (SoftPanel.Selected_Ramp == RAMP1)
        //LCD_PrintCString(RampModes[EditBuffer[EB_RAMP1_MODE]]);
        lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(RampModes[EditBuffer[EB_RAMP1_MODE]]))));
      else
        //LCD_PrintCString(RampModes[EditBuffer[EB_RAMP2_MODE]]);
        lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(RampModes[EditBuffer[EB_RAMP2_MODE]]))));

      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 15 + LCD_Offset);
      lcd.print(F("|")); // mise en page
      //LCD_PrintCString(MMOD_Sources[EditBuffer[EB_TRACK_INPUT]]);
      lcd.print(strcpy_P(bufferProgmem, (PGM_P)pgm_read_word(&(MMOD_Sources[EditBuffer[EB_TRACK_INPUT]]))));
      break;
    ...
    case SOFT_PAGE3:
      ...
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + LCD_Offset);
      LCD_PrintBCD2(EditBuffer[EB_TRACK_POINT1]);
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 4 + LCD_Offset);
      LCD_PrintBCD2(EditBuffer[EB_TRACK_POINT2]);
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 9 + LCD_Offset);
      LCD_PrintBCD2(EditBuffer[EB_TRACK_POINT3]);
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 14 + LCD_Offset);
      LCD_PrintBCD2(EditBuffer[EB_TRACK_POINT4]);
      LCD_CursorSet(LCD_SECONDROWSTARTPOSITION + 18 + LCD_Offset);
      LCD_PrintBCD2(EditBuffer[EB_TRACK_POINT5]);
...
void handle_matrix_page1()
{
  ...
  if (SoftPanel.Button == SOFT_EDIT_5) // send midi when this button is pushed
  {
    ...
    source = EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Source];
    dest = EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Destination];
    amount = EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Amount];
  ...
  if (SoftPanel.EncoderValue != 0)
  {
    switch (SoftPanel.EditButton)
    {
      ...
      case Edit2: // matrix mod source
        SetEncoderValue(&EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Source], 20);
        break;

      case Edit3: // matrix mod dest
        SetEncoderValue(&EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Destination], 32);

        break;

      case Edit5: // matrix mod amount, dont send midi while we turn the encoder, matrix takes to long to respond if we do.
        SetEncoder7Signed(&EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Amount]);
        //                SetEncoderValue(&EditBuffer[EB_MmodMap[SoftPanel.Selected_MatrixBus].Amount], 255);
  ...
  if (SoftPanel.EncoderValue != 0)
  {
    switch (SoftPanel.EditButton)
    {
      case Edit2: // ramp rate
        ...
        {
          param = SX_RAMP1_RATE;
          SetEncoderValue(&EditBuffer[EB_RAMP1_RATE], 63);
          value = EditBuffer[EB_RAMP1_RATE];
        }
        else
        {
          param = SX_RAMP2_RATE;
          SetEncoderValue(&EditBuffer[EB_RAMP2_RATE], 63);
          value = EditBuffer[EB_RAMP2_RATE];
        }
      ...
      case Edit3: // ramp trigger modes
        ...
        {
          param = SX_RAMP1_MODE;
          SetEncoderValue(&EditBuffer[EB_RAMP1_MODE], 3);
          value = EditBuffer[EB_RAMP1_MODE];
        }
        else
        {
          param = SX_RAMP2_MODE;
          SetEncoderValue(&EditBuffer[EB_RAMP2_MODE], 3);
          value = EditBuffer[EB_RAMP2_MODE];
        }
        break;

      case Edit5: // tracking generator input
        SetEncoderValue(&EditBuffer[EB_TRACK_INPUT], 20);
//        ++EditBuffer[EB_TRACK_INPUT];
//        if (EditBuffer[EB_TRACK_INPUT] > 20)
//          EditBuffer[EB_TRACK_INPUT] = 0;
        param = SX_TRACK_INPUT;
        value = EditBuffer[EB_TRACK_INPUT];
...
void handle_matrix_page3()
{
  ...
  if (SoftPanel.EncoderValue != 0)
  {
    ...
    {
      case Edit1://point 1
        SetEncoderValue(&EditBuffer[EB_TRACK_POINT1], 63);
        param = SX_TRACK_POINT1;
        value = EditBuffer[EB_TRACK_POINT1];
        break;

      case Edit2: //point 2
        SetEncoderValue(&EditBuffer[EB_TRACK_POINT2], 63);
        param = SX_TRACK_POINT2;
        value = EditBuffer[EB_TRACK_POINT2];
        break;

      case Edit3: //point 3
        SetEncoderValue(&EditBuffer[EB_TRACK_POINT3], 63);
        param = SX_TRACK_POINT3;
        value = EditBuffer[EB_TRACK_POINT3];
        break;

      case Edit4: //point 4
        SetEncoderValue(&EditBuffer[EB_TRACK_POINT4], 63);
        param = SX_TRACK_POINT4;
        value = EditBuffer[EB_TRACK_POINT4];
        break;

      case Edit5: //point 5
        SetEncoderValue(&EditBuffer[EB_TRACK_POINT5], 63);
        param = SX_TRACK_POINT5;
        value = EditBuffer[EB_TRACK_POINT5];
        break;
...
//      case EditDec:
//        --EditBuffer[EB_TRACK_INPUT];
//        if (EditBuffer[EB_TRACK_INPUT] == 255)
//          EditBuffer[EB_TRACK_INPUT] = 0;
//        param = EB_TRACK_INPUT;
//        value = EditBuffer[EB_TRACK_INPUT];
//        break;
