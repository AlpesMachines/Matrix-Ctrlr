...
//unsigned char EditBufferOrigA [134];
//unsigned char EditBufferOrigB [134];
//unsigned char EditBufferOrigC [134];
//unsigned char EditBufferOrigD [134];
...
// Constantes
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
const unsigned char singlePatchDataFormatCC[97][9] PROGMEM = {
...
void InitEditBuffer()
{
  ...
  for (i = 0; i < 134; i++) {
    EditBuffer[i] =  pgm_read_byte_near(&DefaultEditBuffer[i]);
    EditBufferOrig[i] = pgm_read_byte_near(&DefaultEditBuffer[i]);
  }
...
/////////////////////////////////////////////////////////////////////////////
void SendEditBuffer(unsigned char  interface)
{
  unsigned char i;
  unsigned char checksum = 0;
  ...
  for (i = 0; i < 134; i++) {
    // transmit the parameter value
    sysex[5 + 2 * i] = EditBuffer[i] & 0x0f;
    sysex[5 + 2 * i + 1] = (EditBuffer[i] >> 4) & 0x0f;
...
void SendSingleUnison(unsigned char interface, unsigned char number)
{
  ...
  sysex[5] = UnisonDetune & 0x0f;
  sysex[6] = (UnisonDetune >> 4) & 0x0f;

  // checksum version
  checksum += UnisonDetune;
...
void DumpCtrlrBank(unsigned char interface, unsigned char bank)
{
  ...
  for (unsigned char k = 0; k < 100; k++)
  {
    ...
    Read_Patch_From_BS(bank, k); // read the patch
